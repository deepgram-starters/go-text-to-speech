...
Go demo app for Deepgram Text-to-Speech.
...
| File | Purpose |
|------|---------|
| `main.go` | Main backend — API endpoints and request handlers |
...
```
feat(go-text-to-speech): add diarization support
fix(go-text-to-speech): resolve WebSocket close handling
refactor(go-text-to-speech): simplify session endpoint
